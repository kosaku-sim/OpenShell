...
impl NetworkNamespace {
    ...
    pub fn install_bypass_rules(&self, proxy_port: u16) -> Result<()> {
        ...
        //
        // Use broad MASQUERADE + FORWARD rules for any TCP 443 from sandbox.
        // The sandbox-side iptables ACCEPT rules (per-IP) are the real gate;
        // the host side just needs to be able to forward whatever the sandbox
        // allows. This avoids having to update host-side rules when DNS
        // round-robin returns new IPs.
        let direct_tcp_hosts = parse_direct_tcp_hosts();
        if !direct_tcp_hosts.is_empty() {
            let sandbox_cidr = format!("{}/32", self.sandbox_ip);

            // Broad MASQUERADE: any TCP 443 from sandbox IP
            let _ = Command::new(&iptables_path)
                .args([
                    "-t", "nat", "-A", "POSTROUTING",
                    "-s", &sandbox_cidr,
            ...
            // Broad FORWARD: any TCP 443 from sandbox IP
            let _ = Command::new(&iptables_path)
                .args([
                    "-A", "FORWARD",
                    "-s", &sandbox_cidr,
